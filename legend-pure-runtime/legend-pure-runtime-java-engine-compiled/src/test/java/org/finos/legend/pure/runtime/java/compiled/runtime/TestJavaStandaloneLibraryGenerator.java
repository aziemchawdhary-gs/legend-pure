...
public class TestJavaStandaloneLibraryGenerator extends AbstractPureTestWithCoreCompiled
{
    ...
    public void testGenerateOnly_allRepos() throws Exception
    {
        ...
        Pattern pattern = Pattern.compile("org/finos/legend/pure/generated/(CoreGen|PureCompiledLambda|PureEnum(_(LazyImpl|LazyComponent))?|(Package_(Impl|OverrideImpl|LazyImpl|LazyConcrete|LazyVirtual))|((test|platform|other)_\\w++)|(Root_(meta|test|other)_\\w++(\\$\\w++)?))\\.java");
    ...
    public void testGenerateOnly_oneRepo() throws Exception
    {
        ...
        List<Path> files = Files.walk(sourcesDir).filter(Files::isRegularFile).collect(Collectors.toList());
        Assert.assertNotEquals(Lists.fixedSize.empty(), files);
        Pattern pattern = Pattern.compile("org/finos/legend/pure/generated/((CoreGen|PureCompiledLambda|PureEnum(_(LazyImpl|LazyComponent))?)|(test_\\w++(\\$\\w++)?)|(Root_test_\\w++(\\$\\w++)?))\\.java");
